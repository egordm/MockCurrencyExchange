...
class BalanceRepository extends PresentableRepository
{
    ...
    /*
     * gets the total balance in dollars
     *
     *
     */
    public function getConvertedBalance(\Illuminate\Database\Eloquent\Collection $balances)
    {
        $convertedbalance = 1;
        foreach ($balances as $balance):
            if (!($balance->valuta->name == 'US Dollar')){
                ...
                $order = DB::table('orders')->where('valuta_pair_id', $valutapair)->Orderby('created_at', 'desc')->first();
                $conversion = $conversion = $order->price/$order->quantity;
